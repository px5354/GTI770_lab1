...
class MLPClassifierTensorBoard(object):
    def __init__(self, number_of_classes, batch_size, image_size, number_of_channels, number_of_epochs,
                 dropout__keep_probability, learning_rate, number_of_hidden_layer, train_path):
    ...
    def train(self, dataset):
        ...
        with tf.Session(graph=tf.Graph()) as sess:
            ...
            def feed_dict(x, y_, keep_prob, train, dataset):
            ...
            for epoch in range(self.number_of_epochs):
                    if epoch % 10 == 0:  # Record summaries and test-set accuracy
                        dict = feed_dict(X, y_, keep_prob, False, dataset)

                        summary, train_accuracy, train_scores, train_prediction = sess.run(
                            [merged, accuracy, scores, correct_prediction], feed_dict=dict)
                        test_writer.add_summary(summary, epoch)
                        print('Accuracy at step %s: %s' % (epoch, train_accuracy))
                        ...
                        if epoch % 100 == 99:  # Record execution stats
                            ...
                            summary, _ = sess.run([merged, train_step],
                                                  feed_dict=feed_dict(X, y_, keep_prob, True, dataset),
                                                  options=run_options,
                            ...
                            summary, _ = sess.run([merged, train_step], feed_dict=feed_dict(X, y_, keep_prob, True, dataset))
                            train_writer.add_summary(summary, epoch)

            saver = tf.train.Saver()
            saver.save(sess, self.train_path + "my_mlp_test")
